...
{"class":t.sPageButton+" "+g,id:0===r&&"string"===typeof b?a.sTableId+"_"+b:null}).append(c("<a>",{href:"#","aria-controls":a.sTableId,"aria-label":u[b],"data-dt-idx":p,tabindex:a.iTabIndex,"class":"page-link"}).html(e)).appendTo(d),a.oApi._fnBindAction(i,{action:b},m),p++)}},i;try{i=c(h).find(d.activeElement).data("dt-idx")}catch(v){}q(c(h).empty().html('<ul class="pagination"/>').children("ul"),s);i!==m&&c(h).find("[data-dt-idx="+i+"]").trigger("focus")};return f});
        ...
        document.getElementById('addRowBtn').addEventListener('click', function() {
            ...
            var stockInfoCell = document.createElement('td');
            var stockInfoSelect = document.createElement('select');
            stockInfoSelect.name = 'stockInfo[]';
            stockInfoSelect.innerHTML = `
                <option value=""></option>`;
            stockInfoCell.appendChild(stockInfoSelect);
            newRow.appendChild(stockInfoCell);

            var orderQuantityCell = document.createElement('td');
            var orderQuantitySelect = document.createElement('select');
            orderQuantitySelect.name = 'orderQuantity[]';
            orderQuantitySelect.innerHTML = `
                <option value=""></option>`;
            orderQuantityCell.appendChild(orderQuantitySelect);
            newRow.appendChild(orderQuantityCell);
            
            var recipientNameCell = document.createElement('td');
            var recipientName = document.createElement('input');
            recipientName.type = 'text';
            recipientName.name = 'recipientName[]';
            recipientNameCell.appendChild(recipientName);
            newRow.appendChild(recipientNameCell);

            var recipientAddressCell = document.createElement('td');
            var recipientAddress = document.createElement('input');
            recipientAddress.type = 'text';
            recipientAddress.name = 'recipientName[]';
            recipientAddressCell.appendChild(recipientAddress);
            newRow.appendChild(recipientAddressCell);

            var recipientContactCell = document.createElement('td');
            var recipientContact = document.createElement('input');
            recipientContact.type = 'text';
            recipientContact.name = 'recipientName[]';
            recipientContactCell.appendChild(recipientContact);
            newRow.appendChild(recipientContactCell);

            var noteCell = document.createElement('td');
            var note = document.createElement('input');
            note.type = 'text';
            note.name = 'recipientName[]';
